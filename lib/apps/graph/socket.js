...
var dispatcher = function (conn) {
  conn.on('data', function (packet) {
    ...
    if (packet.channel !== 'handshake') {
      conn.send('handshake', false);
      return conn.close(401, "No handshake provided");
    }

    conn.removeAllListeners('data');

    var sessionKey = packet.message || crypto.pseudoRandomBytes(16).toString('base64').replace(/\=+$/, '');

    conn.send('handshake', sessionKey);

    conn.session = new Session(sessionKey).load();
